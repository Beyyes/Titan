...
function fetchData() {
  let res = null;
  $.ajax('http://DEFINE-MASTER-IP:8088/ws/v1/cluster/apps', {
...
export default class MetricsMonitorPage extends Component {
  ...
  handleClick = (e, appId) => {
    e.preventDefault();
    this.url = `http://DEFINE-MASTER-IP:3333/dashboard/script/spark.js?app=${appId}&maxExecutorId=2&orgId=1&from=now-6h&to=now`;
    if (this.setState) {
