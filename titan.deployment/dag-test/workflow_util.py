...
def create_dist_tensorflow_job_config(job_name, code_dir, task_num, memory_mb, cpu_num, gpu_num, cmd):
    ...
    ps_role["gpuNumber"] = 0
    ps_role[
        "command"] = "cd " + cd + " && " + cmd + " --ps_hosts=$PAI_TASK_ROLE_ps_HOST_LIST --worker_hosts=$PAI_" \
                                                 "TASK_ROLE_worker_HOST_LIST --job_name=ps --task_index=$PAI_CURRENT_TASK_ROLE_CURRENT_TASK_INDEX"
    ...
    worker_role[
        "command"] = "cd " + cd + " && " + cmd + " --ps_hosts=$PAI_TASK_ROLE_ps_HOST_LIST --worker_hosts=$PAI_TASK_ROLE_worker_HOST_LIST " \
...
def submit_job(user_name, password, job_config):
    ...
    r = req.put(platform_job_rest_api_path + '/' + job_name, data=data,
...
def get_yarn_logs(job):
    ...
    if type(job) == dict:
        ...
        if log != None:
            return log.encode('ascii')

        for it in job.items():
            log = get_yarn_logs(it[1])
            if log != None:
                if type(log) == str:
                    logs.append(log)
                else:
...
def poll_job(job_name, interval_sec=60):
    while True:
        ...
        # yarn_logs = get_yarn_logs(job)
        # if yarn_logs: log.info("yarn container logs url: [%s]" % (",\n".join(yarn_logs)))

        sleep(interval_sec)


def poll_job_logs(job_name, interval_sec=60):
    while True:
        job = get_job(job_name)
        status = get_job_status(job).lower()
