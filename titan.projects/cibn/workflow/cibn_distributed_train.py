...
def on_eval_model_yarn(**kwargs):
    ...
    job_config = wu.create_single_tensorflow_job_config(eval_job_guid, train_code_dir, 1, eval_memory_mb, eval_cpu_num, eval_gpu_num, eval_cmd2)
    log.info("job config %s" % job_config)
    wu.submit_job(portal_user_name, portal_user_password, job_config)
...
op_download_train_output_model = PythonOperator(task_id='op_download_train_output_model', dag=dag, python_callable=on_download_train_output_model)
