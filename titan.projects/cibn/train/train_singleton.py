...
def main(_):
    ...
    summary_output_dir = FLAGS.summary_output_dir
 
    # train_file_path = ['hdfs://10.190.148.125:9000/path/test/train_data_val_100.ffm']
    ...
    model_dir = '/home/neilbao/test-model/model'
    #model_dir = 'hdfs://10.190.148.125:9000/path/test/model'
    ...
    train_dataset = tf.data.TextLineDataset(train_file_path).batch(FLAGS.batch_size)
    ...
    #summary_merged = tf.summary.merge(tf.get_collection(tf.GraphKeys.SUMMARIES))
    ...
    num_train_step = int(FLAGS.num_records / FLAGS.batch_size * FLAGS.num_epochs) + 1
    ...
    with tf.Session() as sess:
        summary_writer = tf.summary.FileWriter(FLAGS.summary_output_dir, sess.graph)
        ...
        #while not sess.should_stop():# and epoch < FLAGS.num_epochs:
        while True:
            try:
                ...
                _, step_loss, step_data_loss, g_step, s_m = sess.run([train_opt, loss, data_loss, global_step, summary_merged], feed_dict=build_train_feed_dict(state, train_model_input))
                #_, step_loss, step_data_loss, g_step = sess.run([train_opt, loss, data_loss, global_step], feed_dict=build_train_feed_dict(state, train_model_input))
                step += 1

                summary_writer.add_summary(s_m, g_step)
